...
export function DynamicCollectionClient({
  ...
  const [editingRecord, setEditingRecord] = useState<Entrada | null>(
  ...
  const handleOpenModal = (record: Entrada | null = null) => {
    if (record) {
      ...
      esquema?.campos?.forEach((f: Campo) => {
  ...
  const handleFieldChange = (fieldId: string, value: unknown) => {
    setFormData((prev: Record<string, unknown>) => ({ ...prev, [fieldId]: value }));
  };
  ...
  const handleToggleActive = async (record: Entrada) => {
  ...
  const renderForm = () => (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      >
        {esquema?.campos?.map((field: Campo) => (
          <div key={field.id}>
          ...
          <thead>
            <tr className="bg-purple-900/20 border-b border-purple-500/20 text-[10px] font-bold text-purple-200 uppercase tracking-widest">
              {esquema?.campos?.slice(0, 4).map(
                (
                  f: Campo, // only show up to 4 columns to avoid clutter
                ) => (
              ...
              >
                {esquema?.campos?.slice(0, 4).map((f: Campo) => (
                  ...
                  >
                    {f.type === "boolean" ? (
                      <span
                        className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${c[f.id] ? "bg-green-500/20 text-green-400" : "bg-red-500/20 text-red-400"}`}
                      >
                        {c[f.id] ? "SI" : "NO"}
                      </span>
                    ) : f.type === "image" ? (
